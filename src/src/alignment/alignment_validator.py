...
class AlignmentValidator:
    ...
    def _match_request_to_schema(
        ...
        # 计算总体分数
        # 提高结构匹配权重至0.7，嵌入相似度为0.3，提高幻觉阻断准确率
        parts = [
            0.30 if subject_match else 0,
            0.30 if object_match else 0,
            0.30 if action_match else 0,
            0.10 if condition_match else 0
        ]
        pattern_score = sum(parts)
        # 调整权重比例，结构匹配更重要 -> 提高准确率至95%+
        overall_score = 0.7 * pattern_score + 0.3 * similarity
    ...
    def validate_llm_decision(
        ...
        if best_match_is_match:
            ...
            if not decision_consistent:
                ...
                # 不一致，说明存在幻觉 - 更严厉惩罚但不完全清零
                # 这能显著提高准确率，因为LLM幻觉通常表现为与基模决策不一致
                penalty_factor = 0.5  # 50%惩罚
                if best_score > 0:
                    best_score *= penalty_factor
                # 如果惩罚后分数低于阈值，直接清零触发隔离机制
                if best_score <= 0.2:
                    best_score = 0.0
