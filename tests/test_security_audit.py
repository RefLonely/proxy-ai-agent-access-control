...
from src.security_audit.global_situation import GlobalSituationManager, SituationLevel
from src.security_audit.emergency_response import EmergencyResponseManager, ResponseLevel
...
class TestNodeValidation:
    ...
    def test_node_validation_with_valid_node(self):
        """测试验证有效节点"""
        agent = Agent(agent_id="grid-001", agent_type=AgentType.PHOTOVOLTAIC)
    ...
    def test_node_validation_with_multiple_nodes(self):
        """测试验证多个节点"""
        agents = [
            Agent(agent_id="grid-001", agent_type=AgentType.PHOTOVOLTAIC),
            Agent(agent_id="grid-002", agent_type=AgentType.WIND),
            Agent(agent_id="grid-003", agent_type=AgentType.PLC)
        ]
    ...
    def test_trusted_nodes_retrieval(self):
        """测试获取可信节点列表"""
        agent1 = Agent(agent_id="grid-001", agent_type=AgentType.PHOTOVOLTAIC)
        agent2 = Agent(agent_id="grid-002", agent_type=AgentType.WIND)
...
class TestGlobalSituation:
    ...
    def test_situation_level_detection(self):
        """测试态势级别检测"""
        # 测试安全状态
        safe_situation = SecuritySituation(threat_score=0.2)
        assert safe_situation.level == SituationLevel.SAFE
        assert not safe_situation.is_critical
        
        # 测试低风险状态
        low_situation = SecuritySituation(threat_score=0.35)
